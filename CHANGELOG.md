...
- Auto-normalize Supabase transaction pooler `DATABASE_URL` (`:6543`) with `pgbouncer=true`, `schema=public`, and `connection_limit=1` so Prisma on Vercel does not hit prepared-statement / pool timeout errors when those query params were omitted

## [3.6.3] - 2026-08-10

### Fixed

- Vercel deploy: set `outputDirectory` to `public` and add empty `public/` so API-only builds with a custom `buildCommand` no longer fail with missing output directory

## [3.6.2] - 2026-08-10

### Changed

- `.gitignore` ignores all `.env` / `.env.*` secrets; only `*.example` templates remain tracked

## [3.6.1] - 2026-08-10

### Fixed

- Vercel `buildCommand` uses `bunx prisma generate` so Prisma CLI runs under Bun (bare `prisma` failed with `env: 'node': No such file or directory` on Bun-only builders)

## [3.6.0] - 2026-08-09

### Added

- Optional query param `fields` (comma-separated allowlisted keys) on all data endpoints, including exports and `POST .../search` — sparse fieldsets for `items[]` or top-level object keys; pagination / export `total` envelope always kept
